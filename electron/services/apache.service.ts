...
export class ApacheService {
  ...
  private prepareRuntimeConfig(apacheDir: string, confPath: string, port: number): string {
    const runtimeConfPath = getApacheRuntimeConfigPath();
    const serverRoot = toApachePath(apacheDir);
    const original = fs.readFileSync(confPath, 'utf-8');
    const apacheLogDir = toApachePath(ensureDir(getApacheLogDir()));
    const errorLogPath = `${apacheLogDir}/error.log`;
    const accessLogPath = `${apacheLogDir}/access.log`;
    const pidFilePath = toApachePath(getApachePidFilePath());
    const vhostConfigPath = toApachePath(getApacheVhostConfigPath());
    ...
    patched = this.ensureDirectiveEnabled(patched, `Include "${vhostConfigPath}"`);
    patched = this.replaceOrAppendDirective(
      patched,
      /^ErrorLog\s+.+$/mi,
    ...
    patched = this.replaceOrAppendDirective(
      patched,
      /^CustomLog\s+.+$/mi,
      `CustomLog "${accessLogPath}" common`
    );
    patched = this.replaceOrAppendDirective(
      patched,
      /^#?\s*PidFile\s+.+$/mi,
    ...
    this.ensureDevStackVhostConfig();
    ensureDir(path.dirname(runtimeConfPath));
  ...
  private replaceOrAppendDirective(config: string, pattern: RegExp, directive: string): string {
    if (pattern.test(config)) {
      return config.replace(pattern, directive);
    }
    return `${config.trimEnd()}\n${directive}\n`;
  }

  private ensureDevStackVhostConfig(): void {
    const vhostPath = getApacheVhostConfigPath();
    if (fs.existsSync(vhostPath)) return;

    ensureDir(path.dirname(vhostPath));
    const content = [
