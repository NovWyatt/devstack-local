...
import { RemoteService } from '../electron/services/remote.service.ts';
import type {
...
type FakeBehavior = {
  connect?: (connection: FakeConnection, timeoutMs: number) => Promise<void>;
  list?: (rootPath: string) => Promise<RemoteDirectoryEntry[]>;
  disconnect?: () => Promise<void>;
};

class MemoryStorage {
  private connections: RemoteConnectionRecord[] = [];
  private secrets: RemoteConnectionSecretRecord[] = [];
...
class FakeTransportClient {
  connectCalls: Array<{ connection: FakeConnection; timeoutMs: number }> = [];
  listCalls: string[] = [];
  disconnectCalls = 0;
  ...
  async connect(connection: FakeConnection, timeoutMs: number): Promise<void> {
...
function createHarness(behaviors: FakeBehavior[], connectTimeoutMs: number = 150): {
  service: RemoteService;
  storage: MemoryStorage;
  bridge: FakeProcessBridge;
  clients: FakeTransportClient[];
} {
  const storage = new MemoryStorage();
  const bridge = new FakeProcessBridge();
  const clients: FakeTransportClient[] = [];
...
async function main(): Promise<void> {
  ...
  await runTest('Add, edit, connect, and delete connection', async () => {
    const listEntries: RemoteDirectoryEntry[] = [
    ...
    assert(
      clients[0].connectCalls[0]?.connection.password === 'super-secret',
      'Expected stored password to be preserved across edit'
    );
  ...
  await runTest('Validation rejects invalid connection input', async () => {
    ...
    assert(
      (result.error ?? '').includes('protocol prefix') || (result.error ?? '').includes('Port'),
      `Unexpected validation error: ${result.error ?? result.message}`
    );
  ...
  await runTest('Timeout handling aborts connect and leaves connection in error state', async () => {
    const neverResolvingPromise = new Promise<void>(() => undefined);
    const { service, clients } = createHarness([
      {
    ...
    assert(
      (connectResult.error ?? '').toLowerCase().includes('timed out'),
      `Unexpected timeout error: ${connectResult.error ?? connectResult.message}`
    );
  ...
  await runTest('Disconnect cleanup succeeds even when transport close throws', async () => {
    ...
    const summaries = await service.listConnections();
    assert(
      summaries[0]?.status === 'disconnected',
      `Expected disconnected status after cleanup, got ${summaries[0]?.status ?? 'none'}`
    );
